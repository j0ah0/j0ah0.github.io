...
<style>
  ...
  /* ---- 대화 목록: 신문/잡지 지면처럼, 화면 높이만큼을 한 "페이지"로 써서
     ...
     창이 1000px 이상일 때만 4단, 그 아래로는 1단(중간 2단 단계는 없음). */
  .gb-flow {
  ...
  /* 4단과 1단 사이에 어정쩡한 "2단" 같은 중간 단계를 두지 않는다 - 창이
     1000px 밑으로 내려가면 곧장 1단(넓은 한 칼럼)으로 줄어든다. 700px
     미만(진짜 모바일)에서는 아래에서 타이포그래피/스크롤 방식까지 더 바뀐다. */
